...
// Black & white copies of the action icons, shown when no Claude Code session
// is tracked. The key keeps its own artwork — matching what Ulanzi Studio
// lists in the action bar — and only loses its colour, so "grey = nothing
// detected" reads at a glance. Generated by tools/make-offline-icons.py.
const OFFLINE_PNG = {};
for (const [key, file] of Object.entries({
  ...
  try {
    OFFLINE_PNG[key] = 'data:image/png;base64,' + readFileSync(join(__dirname, '../resources/offline', file)).toString('base64');
