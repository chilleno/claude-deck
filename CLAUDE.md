...
One plugin (`com.antonio.shortcuts.ulanziPlugin/`, UUID `com.antonio.deck.shortcuts`, name "Claude Deck") with 5 actions: Claude Pet (status GIFs, press = jump to session), Option Next + Option OK (answer AskUserQuestion from the deck), Claude Screen Setup (one-press big-screen toggle + hook self-setup panel), Claude Session Screen (big-key display; also hosts the terminal selector).
...
- Option answering: digit via `write text ... newline NO` auto-submits; `OPT.answeredKey` suppresses re-show after deck answers
