...
- Next mirrors the terminal picker (v2.2.0): arrow-key injection via `sendKeysToTty` (`down`/`up`/`tab`/`shift-tab`, iTerm2-only `sendKeyByTtyScript`; `canSendKeys()` gates). Picker rows = options + **"Other" free-text row — never navigate across it**: forward = single Downs, deck wrap = Up-burst back to top. `OPT.tuiIndex` shadows the TUI highlight; serialized flusher (`mirrorPicker`), any failed send latches `mirrorDead` per ask → deck-local cycling; OK digit is absolute so desync can't misselect. Probed: Down/Up move highlight, Enter selects, Enter on an answered multi-question tab = harmless no-op
...
- Session targeting (v2.1.1): **tty-first** — hook records the claude tty in state JSON (`own_tty()`: fd ttyname → `/dev/tty` → ppid-walk to the claude process; hooks run detached so only the walk works), plugin uses `claudeTtyForSession` (recorded tty trusted only while a live claude sits on it) with the old cwd match (`ps` + `lsof -d cwd`) as fallback. Reason: state cwd follows every `cd` the session runs while the process cwd stays at launch dir — pure cwd matching deadened all keys whenever a session worked in a subdir
...
- Hook debug: `touch claude-state/.debug` → `claude-state/events.log` + plugin `log()` lines to `claude-state/.plugin.log` (Studio swallows plugin stdout — this is the only way to see it); delete `.debug` + `.plugin.log` when done
- Studio scripted quit can return "User canceled" (-128) and leave a **zombie**: plugin process running but deck keys unregistered → every key dead. Fix = proper relaunch; verify plugin log shows `connected` + `add` lines per key
